...
open class SkyFloatingLabelTextField: UITextField,UITextFieldDelegate { // swiftlint:disable:this type_body_length
    ...
    var isRightButtonPressed : Bool = false {
        didSet {
            rightButtonView?.tintColor = isRightButtonPressed ? Colors.BLACK : Colors.GRAY
    ...
    var rightImage:UIImage? {
        didSet {
            if let image = rightImage {
                ...
                rightButtonView?.tintColor = Colors.GRAY
                
                let view = UIView(frame: CGRect(x: -6, y: 8, width: 38, height: 40))
    ...
    @IBInspectable dynamic open var selectedTitleColor: UIColor = Colors.GRAY {
        didSet {
            updateTitleColor()
        }
    }
